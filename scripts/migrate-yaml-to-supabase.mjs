...
async function importKPIs() {
  ...
  for (const file of files) {
    try {
      ...
      // Parse industry (could be array or string)
      const industry = Array.isArray(kpiData.Industry) ? kpiData.Industry.join(', ') : (kpiData.Industry || null);
      
      // Parse BI Source System (could be array or string)
      const biSourceSystem = Array.isArray(kpiData['BI Source System']) ? kpiData['BI Source System'].join(', ') : (kpiData['BI Source System'] || null);
      
      // Parse Dashboard Usage (could be array or string)
      const dashboardUsage = Array.isArray(kpiData['Dashboard Usage']) ? kpiData['Dashboard Usage'].join(', ') : (kpiData['Dashboard Usage'] || null);
      
      // Parse Segment Eligibility (could be array or string)
      const segmentEligibility = Array.isArray(kpiData['Segment Eligibility']) ? kpiData['Segment Eligibility'].join(', ') : (kpiData['Segment Eligibility'] || null);
      
      // Parse Related KPIs (could be array or string)
      const relatedKpis = Array.isArray(kpiData['Related KPIs']) ? kpiData['Related KPIs'] : (kpiData['Related KPIs'] ? [kpiData['Related KPIs']] : []);
      
      // Parse Report Attribute (could be array or string)
      const reportAttributes = Array.isArray(kpiData['Report Attribute']) ? kpiData['Report Attribute'].join(', ') : (kpiData['Report Attribute'] || null);
      
      // Parse Calculation Notes (could be array or string)
      const calculationNotes = Array.isArray(kpiData['Calculation Notes']) ? kpiData['Calculation Notes'].join('\n') : (kpiData['Calculation Notes'] || null);
      ...
      const { data, error} = await supabase
        ...
        .insert({
          ...
          industry: industry,
          priority: kpiData.Priority || null,
          core_area: kpiData['Core Area'] || null,
          scope: kpiData.Scope || null,
          kpi_type: kpiData['KPI Type'] || null,
          metric: kpiData.Metric || null,
          aggregation_window: kpiData['Aggregation Window'] ? (Array.isArray(kpiData['Aggregation Window']) ? kpiData['Aggregation Window'].join(', ') : kpiData['Aggregation Window']) : null,
          ga4_implementation: kpiData['GA Events Name'] || null,
          adobe_implementation: kpiData['Adobe Analytics Event Name'] || null,
          amplitude_implementation: kpiData['Amplitude Event Name'] || null,
          data_layer_mapping: kpiData['Data Layer Mapping'] || null,
          xdm_mapping: kpiData['XDM Mapping'] || null,
          dependencies: kpiData.Dependencies || null,
          bi_source_system: biSourceSystem,
          report_attributes: reportAttributes,
          dashboard_usage: dashboardUsage,
          segment_eligibility: segmentEligibility,
          related_kpis: relatedKpis,
          sql_query: kpiData['SQL Query Example'] || null,
          calculation_notes: calculationNotes,
          details: kpiData.Details || null,
        });
