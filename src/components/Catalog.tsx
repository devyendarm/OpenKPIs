...
export default function Catalog({ section }: { section: 'kpis'|'dimensions'|'events'|'metrics'|'dashboards' }) {
  ...
  // All sections use their own route path: /kpis/, /events/, /dimensions/, /metrics-catalog/
  const baseUrl = useBaseUrl(`/${section === 'metrics' ? 'metrics-catalog' : section}`);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Get current user to show their drafts
        ...
        if (userName) {
          ...
          // PostgREST syntax: field.eq.value,field2.eq.value2
    ...
    <div className="catalog-page" style={{marginBottom: '2rem'}}>
      <div className="submit-new-cta">
        <Button
          variant="contained"
          startIcon={<AddIcon />}
          href={section === 'dashboards' ? '/dashboards/submit-dashboard' : useBaseUrl(`/${section}/new`)}
        >
          {section === 'kpis' ? 'Add New KPI' : section === 'dimensions' ? 'Add New Dimension' : section === 'events' ? 'Add New Event' : section === 'metrics' ? 'Add New Metric' : 'Submit New Dashboard'}
        </Button>
      </div>
