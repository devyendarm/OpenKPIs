...
(function () {
  ...
  function ensureToolbar(block) {
    ...
    btnDownload.innerHTML = '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 20h14v-2H5v2zm7-18l-5.5 5.5h3.5V15h4V7.5h3.5L12 2z"/></svg>';
  ...
  document.addEventListener('click', (e) => {
    ...
    if (e.target.classList && e.target.classList.contains('mui-toolbar-btn')) {
      ...
      if (label === 'Copy') {
        (async () => {
          try { await navigator.clipboard.writeText(text); e.target.textContent = 'Copied!'; }
            ...
            document.body.appendChild(ta); ta.select(); try { document.execCommand('copy'); e.target.textContent = 'Copied!'; }
            catch { e.target.textContent = 'Failed'; } finally { document.body.removeChild(ta); }
          }
